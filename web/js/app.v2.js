...
function tsStep(name) {
  var card = document.getElementById(_tsCard);
  if (!card) return;
  var span = document.createElement('span');
  span.className = 'ts';
...
function tsDone(name) {
  var el = document.getElementById('ts-' + name.replace(/\s/g, '_'));
...
function handleWS(msg) {
  switch (msg.type) {
    case 'pong': break;
    case 'thinking': tsCreate(); break;
    case 'tool_start': tsStep(msg.tool); break;
    case 'tool_result':
      if (msg.success) tsDone(msg.tool);
      else if (msg.tool !== 'wait' && msg.tool !== 'keyboard_press' && msg.tool !== 'keyboard_type') tsFail(msg.tool);
      if (msg.tool === 'screenshot' && msg.success) { addCard('assistant', '\u{1F4F7} 截图完成'); }
      if (msg.image) {
        ...
        addCard('assistant', '\u{1F4BB} run_code' + (summary ? '\n' + summary : ''));
      }
      break;
    case 'text_delta':
      tsHide();
...
async function toggleVoice() {
  ...
  try {
    ...
    mediaRecorder.onstop = async function() {
      ...
      var r = new FileReader();
      r.onload = function() { ws.send(JSON.stringify({ type: 'voice', payload: { audio: r.result.split(',')[1] } }));  };
